...
function onDeviceReady3() {
    ...
    var initialEngineBoundsMaxX = render.bounds.max.x*2;
    var initialEngineBoundsMaxY = render.bounds.max.y*2;
    var centerX = -500;
    var centerY = -360;
...
/*
  ...
  //  }
   // var newGround = Bodies.fromVertices(800 - 674, 1220 - 561, newCoorList, {isStatic: true}, true);
    ...
    function accelerate() {
        if (!isStopped) {
            if (myCar != undefined && !isAlmostStopped) {
   ...
   /* $("#pedalFWD").on("mouseover", function() {
    ...
    $.get("img/leveltest1.svg").done(function(data) {
        ...
        var posArray2 = [];
        for (var y = 0; y < 10; y++) {
            var xval = -6200 + 3400*y;
            var yval = getCoinYVal(xval, 700, -1400, 1, theGround);
            if (!yval) {
                break;
            }
